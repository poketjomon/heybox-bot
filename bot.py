...
def do_fetch_posts(session, config):
    ...
    topic_id = config.get("topic_id", "7214")
...
def get_pending_posts():
    ...
    return [p for p in posts if "comment" not in p]
...
def reply_to_post(session, config, prompt, post, dry_run=False):
    ...
    if not dry_run:
        ...
        log(f"[回复帖子] 发送结果: {result.get('msg', result.get('status', ''))}")
...
def reply_to_at(session, config, prompt, msg, dry_run=False):
    ...
    if not dry_run:
        ...
        log(f"[回复@] 发送结果: {result.get('msg', result.get('status', ''))}")
...
def reply_to_reply(session, config, prompt, msg, dry_run=False):
    ...
    post_content = {
        ...
        "content": (
            f"{post_detail.get('content', '')}\n\n"
            f"---\n"
            f"我之前的评论：{msg['my_comment']}\n"
            f"用户「{msg['user_a']}」回复我说：{msg['comment_text']}\n\n"
            f"（注意：请根据对方的回复内容继续对话，不要重复我之前说过的话）"
        ),
        "imgs": post_detail.get("imgs", []),
    }
    ...
    if not dry_run:
        ...
        log(f"[回复评论] 发送结果: {result.get('msg', result.get('status', ''))}")
